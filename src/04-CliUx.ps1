...
function Get-CapsulenvCliCommandCatalog {
    ...
    $script:CapsulenvCliCommandCatalog = @(
        ...
        [pscustomobject][ordered]@{ Category='Inspect'; Name='resolve'; Usage='capsulenv resolve <app> [--json]'; Summary='Resolve a trusted host-local or portable program realization without mutating it.'; Topic=''; Hidden=$false },
        ...
        [pscustomobject][ordered]@{ Category='Lifecycle'; Name='migrate'; Usage='capsulenv migrate [--browser <app>] [--powershell-profile <path>] [--powershell-history <path>] [--force]'; Summary='Explicitly migrate supported legacy state into portable State.'; Topic='migration'; Hidden=$false },
        ...
        [pscustomobject][ordered]@{ Category='Lifecycle'; Name='routine'; Usage='capsulenv routine <list|run> [...]'; Summary='Inspect or invoke capsule-local lifecycle routines.'; Topic='routine'; Hidden=$false },
...
function Get-CapsulenvCliActionCatalog {
    ...
    if ($null -eq $script:CapsulenvCliActionCatalog) {
        $script:CapsulenvCliActionCatalog = @(
            ...
            [pscustomobject][ordered]@{ Group='app'; Name='exec'; Usage='capsulenv app exec <installed-app> <bin> [-- arguments...]'; Summary='Execute an installed app bin through the provider-neutral runtime resolver.' },
            [pscustomobject][ordered]@{ Group='routine'; Name='list'; Usage='capsulenv routine list'; Summary='Show configured lifecycle routines and last-run state.' },
            [pscustomobject][ordered]@{ Group='routine'; Name='run'; Usage='capsulenv routine run <OnEnter|OnExit|OnRehydrate|OnEject> [name] [--force]'; Summary='Invoke one lifecycle trigger, honoring interval state unless forced.' },
