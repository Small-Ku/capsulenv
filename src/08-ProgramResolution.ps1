...
function Get-CapsulenvProgramCandidates {
    ...
    foreach ($selector in $selectors) {
        ...
        $candidate = New-CapsulenvProgramCandidate -Name $Requirement.Name -Executable $executable -Root $installed.Current -Provider $provider -Scope ([string]$selector.Scope) -Version ([string]$installed.Manifest.version) -Trusted:$trust.Trusted -OwnsLifecycle:($provider -eq 'capsulenv-local') -Provenance ([string]$installed.Selector)
