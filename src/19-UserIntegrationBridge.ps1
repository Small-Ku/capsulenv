...
function Get-CapsulenvCapsuleLocatorCandidates {
    ...
    foreach ($locator in @($Bridge.CapsuleLocators)) {
        $relative = [string]$locator.RelativePath
...
function Resolve-CapsulenvUserIntegrationCapsuleRoot {
    ...
    foreach ($root in @(Get-CapsulenvCapsuleLocatorCandidates -Bridge $bridge)) {
...
$capsuleRoot = $null
foreach ($candidate in @($manifest.CapsuleRoots) + @($manifest.CapsuleLocators | ForEach-Object {
    foreach ($drive in @(Get-PSDrive -PSProvider FileSystem -ErrorAction SilentlyContinue)) {
...
'@
    ...
    $manifest = [pscustomobject][ordered]@{
        ...
        CapsuleLocators = @([ordered]@{
            RelativePath = $rootReference.Substring([System.IO.Path]::GetPathRoot($rootReference).Length).TrimStart([char[]]'\/')
        })
