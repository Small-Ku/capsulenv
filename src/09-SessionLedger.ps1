...
function Initialize-CapsulenvSession {
    ...
    if (-not $ownershipWasExplicit) {
        if ($processWasExplicit) {
            throw 'Session identity initialization does not accept a ProcessId; register an attached process or use the owned launch boundary.'
        }
...
function Get-CapsulenvLedgerOwnedProcessRecord {
    ...
    $sessionId = if ($null -ne $RequestedRecord.PSObject.Properties['SessionId']) { [string]$RequestedRecord.SessionId } else { '' }
    $processNonce = if ($null -ne $RequestedRecord.PSObject.Properties['ProcessNonce']) { [string]$RequestedRecord.ProcessNonce } else { '' }
    $pid = if ($null -ne $RequestedRecord.PSObject.Properties['PID']) { [int]$RequestedRecord.PID } else { 0 }
    $startIdentity = if ($null -ne $RequestedRecord.PSObject.Properties['ProcessStartIdentity']) { [string]$RequestedRecord.ProcessStartIdentity } else { '' }
    if (
