    ...
    It 'holds an exclusive lease at the OS handle and releases it for reuse' {
        ...
        try {
            ...
            $session = & $script:Module {
                param($CapsuleRoot)
                Initialize-CapsulenvContext -Root $CapsuleRoot | Out-Null
                Initialize-CapsulenvSession -Role lease-test -Provenance test
            } $temporaryRoot
            $first = Acquire-CapsulenvStateLease -StatePath $statePath -Policy exclusive -SessionId $session.SessionId
            $first.Acquired | Should -BeTrue
            {
                Acquire-CapsulenvStateLease -StatePath $statePath -Policy exclusive -SessionId $session.SessionId
            } | Should -Throw
            $second = Release-CapsulenvStateLease -Lease $first
            $second.Released | Should -BeTrue
            $replacement = Acquire-CapsulenvStateLease -StatePath $statePath -Policy exclusive -SessionId $session.SessionId
            try {
