Describe 'Capsulenv host-local UserIntegration bridge' {
    ...
    It 'creates a host-local bridge without embedding removable executable or profile paths' {
        ...
        try {
            ...
            $result.Bridge.BridgePath | Should -BeLike ((Join-Path $temporaryRoot 'host-state') + '*')
    ...
    It 'publishes a package locator without requiring a browser bridge' {
        $temporaryRoot = Join-Path $TestDrive ('capsulenv-package-only-' + [Guid]::NewGuid().ToString('N'))
        $oldStateRoot = $env:CAPSULENV_HOST_STATE_ROOT
        try {
            $env:CAPSULENV_HOST_STATE_ROOT = Join-Path $temporaryRoot 'host-state'
            $result = & $script:Module {
                ...
                [pscustomobject]@{
                    ...
                    BrowserBridge = Get-CapsulenvUserIntegrationBridge -CapsuleId $capsuleId
                    Locator = Get-CapsulenvUserIntegrationCapsuleLocator -CapsuleId $capsuleId
                    ResolvedRoot = Resolve-CapsulenvUserIntegrationCapsuleRoot -CapsuleId $capsuleId
                    ScriptText = Get-Content -LiteralPath $packageBridge -Raw
                }
            } (Join-Path $temporaryRoot 'capsule') (Join-Path $temporaryRoot 'nvme')

            $result.BrowserBridge | Should -BeNullOrEmpty
            $result.Locator.CapsuleId | Should -Be $result.CapsuleId
            $result.Locator.CapsuleRoots | Should -Contain (Join-Path $temporaryRoot 'capsule')
            $result.ResolvedRoot | Should -BeLike '*capsule*'
            $result.ScriptText | Should -Match 'capsule-locator\.json'
            $result.ScriptText | Should -Not -Match 'bridge\.json'
        } finally {
            if ($null -eq $oldStateRoot) { Remove-Item Env:CAPSULENV_HOST_STATE_ROOT -ErrorAction SilentlyContinue } else { $env:CAPSULENV_HOST_STATE_ROOT = $oldStateRoot }
        }
    }
