...
- Provisioning and runtime are separate. Runtime consumers resolve provider-neutral installed app selectors (`capsule/<app>` for Capsulenv ownership, `scoop/<app>` for upstream Scoop) and installed metadata/projections rather than hard-coding provisioning history. Scoop `User`/`Global` is a provider-local scope, exposed only as `scoop:user/<app>` / `scoop:global/<app>` when disambiguation is required; legacy `user/` / `global/` aliases may remain accepted but must not define the primary model.
