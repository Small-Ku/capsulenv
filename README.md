...
這裡指定的是**已安裝 Scoop app selector**，不是 Capsulenv browser preset；同名 app 同時存在 user/global root 時可寫 `user/firefox` 或 `global/librewolf`。`Browsers` 設定只補上 Gecko 特有的 profile path／launch argument，因此自訂 bucket 或改名 manifest 也可加入一個對應 entry。平常 `browser <app>` 優先使用 installed manifest 公開的 `bin`／shortcut executable；若某個 portable wrapper 不適合充當 Windows running-instance delegation broker，可另設 `DefaultExecutablePath`，只讓 default-browser registration 使用 app 內部的 Gecko executable。profile 資料仍由 Scoop `persist` 擁有，但 runtime command 會優先使用 installed manifest 所建立的 `current\<persist-source>` link，使 wrapper 與 Windows delegation 對同一 profile 使用相同 app-visible path。`install-user`／`enable-user`／`user-shell` 會建立 Capsulenv-owned 的 `http`、`https`、`.htm`、`.html` registration；若這台機器已經處於 User mode，普通 `capsulenv.cmd` shell activation 也會同步後來修改的 `DefaultBrowser` 設定。URL delegation 會保留明確 profile 並使用 Gecko 的 `-url` 形式（不把與 `-profile` 不相容的 `-osint` 混入），然後在尚未選中時開啟該 application 的 Windows **Default Apps** 頁。現代 Windows 11 不允許一般程式可靠地靜默覆寫 `http/https` 的 `UserChoice`，所以最後的 **Set default** 仍由使用者在 Settings 確認；Capsulenv 不偽造 association hash。`restore-user` 若發現 Capsulenv browser 仍是 default，會先要求改選另一個 browser，再移除自己精確記錄的 registration。
...
Capsulenv 的 maintenance/control plane 與工作 shell 分開：`install.cmd`、`capsulenv.cmd` 和 capsule 的 `scoop.cmd` gateway 固定由 Windows PowerShell 5.1 執行，完成 activation/rehydrate 後 `shell`／`user-shell` 才另行啟動 capsule Scoop 的 PowerShell 7。這樣 reset/update `pwsh` 時 control process 不會鎖住自己的 `pwsh.exe`／shim。User mode 的 automatic rehydrate 若遇 LibreWolf 等正在使用的 app，該 app reset 會延後而不拖垮整批，下一次 activation 自動重試；ShellOnly 只重建 capsule-owned `current`／shim／persist links，不 materialize Start Menu shortcut 或 manifest environment。需要手動 reset 時：
