...
這裡指定的是**已安裝 Scoop app selector**，不是 Capsulenv browser preset；同名 app 同時存在 user/global root 時可寫 `user/firefox` 或 `global/librewolf`。`Browsers` 設定只補上 Gecko 特有的 profile path／launch argument，因此自訂 bucket 或改名 manifest 也可加入一個對應 entry。`install-user`／`enable-user`／`user-shell` 會建立 Capsulenv-owned 的 `http`、`https`、`.htm`、`.html` registration；若這台機器已經處於 User mode，普通 `capsulenv.cmd` shell activation 也會同步後來修改的 `DefaultBrowser` 設定。handler 直接綁定該 app 的實體 Gecko executable 與 Scoop-persisted profile；URL delegation 會保留明確 profile 並使用 Gecko 的 `-url` 形式（不把與 `-profile` 不相容的 `-osint` 混入），然後在尚未選中時開啟該 application 的 Windows **Default Apps** 頁。現代 Windows 11 不允許一般程式可靠地靜默覆寫 `http/https` 的 `UserChoice`，所以最後的 **Set default** 仍由使用者在 Settings 確認；Capsulenv 不偽造 association hash。`restore-user` 若發現 Capsulenv browser 仍是 default，會先要求改選另一個 browser，再移除自己精確記錄的 registration。
